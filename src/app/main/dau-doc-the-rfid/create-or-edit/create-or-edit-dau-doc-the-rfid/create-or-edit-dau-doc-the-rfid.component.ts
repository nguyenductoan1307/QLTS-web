...
import {
  ...
  ThongTinHongDto, ThongTinHuyDto,
  ThongTinMatDto, ThongTinSCBDDto,
  ThongTinSDDto, ThongTinThanhLyDto,
  ViTriTSDto
...
@Component({
  selector: 'app-create-or-edit-dau-doc-the-rfid',
  templateUrl: './create-or-edit-dau-doc-the-rfid.component.html',
  styleUrls: ['./create-or-edit-dau-doc-the-rfid.component.scss'],
  animations: [appModuleAnimation()],
})
export class CreateOrEditDauDocTheRfidComponent extends AppComponentBase implements OnInit {
  ...
  constructor(private _fb: FormBuilder,
    ...
    private _dauDocTheRFIDServiceProxy: DauDocTheRFIDServiceProxy,
  ...
  ngOnInit(): void {
    ...
    this._dauDocTheRFIDServiceProxy.getUserDangNhap().subscribe((result) => {
      this.userDangNhap = result;
    });

    this._dauDocTheRFIDServiceProxy.getAllNhaCC().subscribe(result => {
      this.listNhaCC = result;
    });

    this._dauDocTheRFIDServiceProxy.getAllLoaiTS().subscribe(result => {
    ...
    if (!this.id) {
      ...
      this.isEdit = true;
      // Sửa
      this._dauDocTheRFIDServiceProxy.getForEdit(this.id, this.isView).subscribe(item => {
        this.demoDto = item;
    ...
    this.form.controls.loaiTaiSan.setValue(this.loaiTaiSanItems.find(w => w.id === 1));
  }
  ...
  // Bấm tải file upload
  onDownloadFile(url) {
    this._dauDocTheRFIDServiceProxy.downloadFileUpload(url).subscribe(result => {
  ...
  save() {
    if (CommonComponent.getControlErr(this.form) === '') {
      ...
      this._dauDocTheRFIDServiceProxy.createOrEdit(this.createInputDto).pipe(
        finalize(() => {
          this.saving = false;
        })
      ).subscribe((result) => {
