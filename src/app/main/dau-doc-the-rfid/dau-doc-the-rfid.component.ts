...
@Component({
  selector: 'app-dau-doc-the-rfid',
  templateUrl: './dau-doc-the-rfid.component.html',
  styleUrls: ['./dau-doc-the-rfid.component.scss'],
  animations: [appModuleAnimation()],
})
export class DauDocTheRfidComponent extends AppComponentBase implements OnInit {
  ...
  getDataPage(isSearch: boolean, lazyLoad?: LazyLoadEvent) {
    this.selectedList = [];
    this.loading = true;
    this._dauDocTheRFIDServiceProxy.getAll(
  ...
  private _showCreateOrEditDemoDialog(id?: number, isView = false): void {
    ...
    createOrEditUserDialog = this._modalService.show(
      CreateOrEditDauDocTheRfidComponent,
      {
        class: 'modal-xl',
  ...
  delete(record: GetAllOutputDto) {
    if (record.trangThaiId === 0) {
      ...
      this.swal.fire({
        ...
        if (result.value) {
          this._dauDocTheRFIDServiceProxy.deleted(record.id).subscribe(() => {
      ...
      this._thongBao('Tài sản ' + record.tenTS + ' sẽ bị xóa!', 'Đầu đọc di dộng đang có rằng buộc dữ liệu, không được xóa!');
    }
  ...
  xoaList(res: GetAllOutputDto[]) {
    ...
    if (this.listIdXoa.length > 0) {
      ...
      this.swal.fire({
        ...
        if (result.value) {
          this._dauDocTheRFIDServiceProxy.xoaList(this.listIdXoa).subscribe(() => {
            this.showDeleteMessage();
            this.getDataPage(false);
          });
        }
  ...
  private _showImportDemoDialog(): void {
    ...
    importExcelDialog.content.onDownload.subscribe(() => {
      this._dauDocTheRFIDServiceProxy.downloadFileMau().subscribe(result => {
    ...
    importExcelDialog.content.onSave.subscribe((ouput) => {
      ...
      this._http.post(URL, formdata).subscribe((res) => {
        this._dauDocTheRFIDServiceProxy.importFileExcel(res['result'][0]).subscribe((message) => {
  ...
  exportToExcel() {
    ...
    this._dauDocTheRFIDServiceProxy.exportToExcel(this.input).subscribe((result) => {
      this._fileDownloadService.downloadTempFile(result);
