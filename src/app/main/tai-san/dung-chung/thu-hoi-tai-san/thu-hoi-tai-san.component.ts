...
import { ThemThuHoiTaiSanComponent } from './them-thu-hoi-tai-san/them-thu-hoi-tai-san.component';
import { LazyLoadEvent } from 'primeng/api';

@Component({
  selector: 'app-thu-hoi-tai-san',
  templateUrl: './thu-hoi-tai-san.component.html',
  styleUrls: ['./thu-hoi-tai-san.component.scss']
})
export class ThuHoiTaiSanComponent extends AppComponentBase implements OnInit {
  ...
  ngOnInit(): void {
    this.khoiTaoForm();
    if (this.count > 0) {
      this.showExistMessage('Danh sách tài sản đã chọn chứa tài sản không được phép thu hồi');
    }
    forkJoin(
    ...
    if (CommonComponent.getControlErr(this.form) === '') {
      ...
      this._taiSanChuaSuDungServiceProxy.capPhatTaiSan(this.createInputDto).pipe(
        ...
        if (result === 0) {
          ...
          this.showSuccessMessage('Thu hồi thành công');
          this._sendMail.sendMailThuHoi(undefined, this.records).subscribe(rs => {});
  ...
  private _getValueForSave() {
    ...
    this.createInputDto = {
      ...this.form.value,
      phanLoaiId: this.enums.TrangThaiTaiSanConst.ThuHoi,
