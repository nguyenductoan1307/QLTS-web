...
import {
  ...
  TaiSanMatServiceProxy,
  ViewTaiSanSuaChuaBaoDuong as ViewTaiSanMat,
...
import { ThemTaiSanMatComponent } from './../them-tai-san-mat/them-tai-san-mat.component';

@Component({
  selector: 'app-create-or-edit-tai-san-mat',
  templateUrl: './create-or-edit-tai-san-mat.component.html',
  styleUrls: ['./create-or-edit-tai-san-mat.component.scss'],
  animations: [appModuleAnimation()],
})
export class CreateOrEditTaiSanMatComponent extends AppComponentBase implements OnInit {
  ...
  save(): void {
    if (CommonComponent.getControlErr(this.form) === '') {
      ...
      this._taiSanServiceProxy.createTaiSanMat(this.createInputDto).pipe(
        finalize(() => {
          this.saving = false;
        })
      ).subscribe((result) => {
        if (result === 1) {
          ...
          this._sendMail.sendMailBaoMat(undefined, this.records).subscribe(rs => {});
  ...
  private _getValueForSave() {
    ...
    this.createInputDto = {
      ...this.form.value,
      phanLoaiId: this.enums.TrangThaiTaiSanConst.TaiSanMat,
