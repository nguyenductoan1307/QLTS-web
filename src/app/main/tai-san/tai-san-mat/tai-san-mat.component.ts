...
import { ViewTaiSanSuaChuaBaoDuong, LookupTableDto, LookupTableServiceProxy, ViewTaiSanMat, TaiSanMatServiceProxy } from '@shared/service-proxies/service-proxies';
...
import { CreateOrEditTaiSanMatComponent } from './create-or-edit-tai-san-mat/create-or-edit-tai-san-mat.component';
import { PermissionTreeEditModel } from '@shared/dft/components/permission-tree-edit.model';

@Component({
  selector: 'app-tai-san-mat',
  templateUrl: './tai-san-mat.component.html',
  styleUrls: ['./tai-san-mat.component.scss'],
  animations: [appModuleAnimation()],
})
export class TaiSanMatComponent extends AppComponentBase implements OnInit, OnDestroy, OnChanges {

  constructor(injector: Injector,
    private _modalService: BsModalService,
    private _taiSanServiceProxy: TaiSanMatServiceProxy,
  ...
  arrTaiSanChecked: ViewTaiSanMat[] = [];
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes.isActive?.currentValue === 4) {
  ...
  getDataPage(isSearch: boolean, lazyLoad?: LazyLoadEvent) {
    this.arrTaiSanChecked = [];
    this.loading = true;
    this._taiSanServiceProxy.getAllTaiSanMat(
  ...
  deleteArrTaiSanChecked() {
    ...
    this.swal.fire({
      title: 'Bạn có chắc chắn không?',
      text: 'Danh sách tài sản đã chọn sẽ được hoàn tác về danh sách trước khi khai báo mất',
      icon: 'warning',
      showCancelButton: true,
      confirmButtonColor: this.confirmButtonColor,
      cancelButtonColor: this.cancelButtonColor,
  ...
  clearArrChecked(arrTaiSanChecked) {
    this._taiSanServiceProxy.hoanTacTaiSanMat(arrTaiSanChecked.map(m => { return { id: m.id }; })).subscribe(() => {
      this.arrTaiSanChecked = [];
