...
import {
  ...
  TaiSanHongServiceProxy,
  ViewTaiSanSuaChuaBaoDuong as ViewTaiSanHong,
...
import { ThemTaiSanHongComponent } from './../them-tai-san-hong/them-tai-san-hong.component';
import { appModuleAnimation } from '@shared/animations/routerTransition';

@Component({
  selector: 'app-create-or-edit-tai-san-hong',
  templateUrl: './create-or-edit-tai-san-hong.component.html',
  styleUrls: ['./create-or-edit-tai-san-hong.component.scss'],
  animations: [appModuleAnimation()],
})
export class CreateOrEditTaiSanHongComponent extends AppComponentBase implements OnInit {
  ...
  constructor(
    ...
    private _sesndMail: MailServerServiceProxy
  ) {
    super(injector);
  }
  ...
  save(): void {
    if (CommonComponent.getControlErr(this.form) === '') {
      ...
      this._taiSanServiceProxy.createTaiSanHong(this.createInputDto).pipe(
        finalize(() => {
          this.saving = false;
        })
      ).subscribe((result) => {
        if (result === 1) {
          ...
          this._sesndMail.sendMailBaoHong(undefined, this.records).subscribe(rs => {});
  ...
  private _getValueForSave() {
    ...
    this.createInputDto = {
      ...this.form.value,
      phanLoaiId: this.enums.TrangThaiTaiSanConst.TaiSanHong,
